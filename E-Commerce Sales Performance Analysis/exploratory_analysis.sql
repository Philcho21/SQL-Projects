...
--We find duplicates from the customer table using the email column as each ustomer should have a unique email address.
...
--Check all order stutueses from Orders table for invalid orders(null, unknown, etc)
...
--We check for error pricing by removing items with negative unit price
...
--Inactive Customers(customers tht did not make a purchase)
SELECT 
	c.customer_id,
	c.first_name,
	c.last_name,
...
--Now we look at monthly revenue base on completed orders with valid prices
SELECT 
...
--Average spent per order(ie AVerage order value)
SELECT 
...
--Break down average order value by country
...
-- Customers at churn risk (no order in 90+ days)
SELECT
    c.customer_id,
    c.first_name || ' ' || c.last_name AS customer_name,
    c.email,
...
-- Products with high stock but low sales volume
SELECT
    p.product_id,
    p.name,
    p.category,
